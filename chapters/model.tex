...
The \emph{module require} rule in Equation~\ref{eq:module-require} matches a program with a \racket{require} expression in the module at the top of the evaluation stack and evaluates it by removing the \racket{require} expression from the module and pushing the required module onto the evaluation stack with the phase shifted appropriately.
...
The \emph{var ref} rule in Equation~\ref{eq:var-ref} looks up a variable in the heap and replaces the variable with its current value.
...
The \emph{add} rule in Equation~\ref{eq:add} replaces an addition expression of numbers with the result of computing their sum.
...
The \emph{set!} rule in Equation~\ref{eq:set} installs a value for a variable into the heap and reduces to the value.
...
When an expression is a value, the \emph{expression done} rule in Equation~\ref{eq:expression-done} matches and removes the expression from the module.
...
When there are no more expressions left in a module, the \emph{module done} rule in Equation~\ref{eq:module-done} applies by removing the module from the program and placing a reference to it in the list of finished modules.
...
The \emph{module done already} rule in Equation~\ref{eq:module-done-already} applies when the current module on the stack is in the finished list, so that modules are not evaluated multiple times. 
