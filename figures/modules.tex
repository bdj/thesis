...
\node (while-lang) [module] [left=of obj-run] {while-lang}
  edge [->] node[auto]{\racket{require}} (obj-run)
  edge [->] node[auto]{\racket{for-syntax}} (obj-comp);
\node (while-test) [module] [below=of while-lang] {while-test}
  edge [->] node[auto]{\texttt{\#lang}} (while-lang) 
  edge [->, bend right] node[swap,auto]{\racket{require}} (obj-run.south);
\end{tikzpicture}
